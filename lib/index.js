...
const logger = new writeln_1.Writeln('Shh');
const secretsManager = new aws_sdk_1.SecretsManager({ region: 'eu-west-1' });
const jsonRegex = /{".+":.+}/;
function syncShh(filePath, pattern) {
    let config;
    (async function () {
        config = await shh(filePath, pattern);
    })();
    deasync_1.default.loopWhile(() => !config);
    return config;
}
exports.syncShh = syncShh;
async function shh(filePath, pattern = /^secret:(.+)$/) {
